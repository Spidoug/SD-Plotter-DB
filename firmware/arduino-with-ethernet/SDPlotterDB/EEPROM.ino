void checkeeprom() {
  ...
  if (vals [2] == 9932) {
    EEPROM.get(addr47, value0);
    EEPROM.get(addr48, value1);
    EEPROM.get(addr49, value2);
    EEPROM.get(addr50, value3);
    value11 = 9934;
  }

  if (vals [2] == 9936) {
    EEPROM.get(addr51, value0);
    EEPROM.get(addr52, value1);
    EEPROM.get(addr53, value2);
    EEPROM.get(addr54, value3);
    EEPROM.get(addr55, value4);
    value11 = 9938;
  }
...
void receeprom() {
  ...
  if (vals [2] == 9872) {
    EEPROM.put(addr47, byte(vals [3]));
    EEPROM.put(addr48, byte(vals [4]));
    EEPROM.put(addr49, byte(vals [5]));
    EEPROM.put(addr50, byte(vals [6]));
    value11 = 9874;
  }

  if (vals [2] == 9876) {
    EEPROM.put(addr51, byte(vals [3]));
    EEPROM.put(addr52, byte(vals [4]));
    EEPROM.put(addr53, byte(vals [5]));
    EEPROM.put(addr54, byte(vals [6]));
    EEPROM.put(addr55, vals [7]);
    value11 = 9878;
  }
...
void write_String(int baseaddress, String message) {
  if (baseaddress + message.length() + 1 > EEPROM.length()) {
    // Evitar escrita fora dos limites da EEPROM
    return;
  }
